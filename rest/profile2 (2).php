...
if($_SESSION['logged_in'] == true){
	...
	switch ($page){
		...
		default; 
				...
				$query=$db->prepare('SELECT avatar,username, fname, email, lname, date FROM membres WHERE id=:id');
				$query->bindValue(':id',$id, PDO::PARAM_INT);
				$query->execute();
...
<head>
  ...
  <title>Mon profile | ClosePlaces</title>
		...
		<?php if(isset($pageOwner)): ?>
			<nav>
				<ul>
					...
					<li><a href="modifier2.php?id=<?php echo $id?>">Modifier photo</a></li>
